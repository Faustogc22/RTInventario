...
Class Consultas {
    ...
    public function ventasfechacliente($fecha_inicio, $fecha_fin, $idcliente) {
        $sql = "SELECT DATE(v.fecha_hora) as fecha,u.nombre as usuario, p.nombre as cliente,v.tipo_comprobante,v.serie_comprobante,v.num_comprobante,v.total_venta,v.impuesto,v.estado FROM venta v INNER JOIN persona p ON v.idcliente=p.idpersona INNER JOIN usuario u ON v.idusuario=u.idusuario WHERE 1=1 ";
        if ($fecha_inicio != '') {
            $sql = $sql . " AND DATE(v.fecha_hora)>='$fecha_inicio'";
        }
        if ($fecha_fin != '') {
            $sql = $sql . " AND DATE(v.fecha_hora)<='$fecha_fin' ";
        }
        if ($idcliente != '') {
            $sql = $sql . " AND v.idcliente='$idcliente'";
        }
        return ejecutarConsulta($sql);
    }
